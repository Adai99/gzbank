...
- (void)uploadFileData:(NSData *)data
{
    ...
    [PPHTTPRequest uploadFileWithParameters:@{@"type":@"file"} name:fileName fileData:data progress:^(NSProgress *progress) {
    } success:^(id response) {
        StrongSelf;
        //        {
        //            datas = "/upload/2018-09-10/1536518363349_20180910023855.png";
        //            msg = "\U4e0a\U4f20\U6210\U529f";
        //            msgCode = 0;
        //        }
        if (response[@"datas"]) {
            strongSelf.item.info = response[@"datas"];
            strongSelf.item.contentServiceInfo = response[@"datas"];
            [SVProgressHUD dismiss];
        }else
        {
            [SVProgressHUD showErrorWithStatus:@"上传未成功"];
            [strongSelf.mutableImages removeAllObjects];
            [strongSelf sw_reloadData];
        }
