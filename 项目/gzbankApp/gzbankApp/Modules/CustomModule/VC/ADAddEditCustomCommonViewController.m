...
#import "ADBaseBtn.h"
#import "ADCellBtn.h"
@interface ADAddEditCustomCommonViewController ()<UITableViewDelegate,UITableViewDataSource,IQActionSheetPickerViewDelegate>
...
- (void)__actionCommit
{
    ...
    if ([self canCommit]) {
        ...
        for (int i = 0; i<self.aryGroup.count; i++) {
            ...
            {
                for (int j = 0; j<groupModel.fieldList.count; j++) {
                    ADCustomCommitSectionModel *sectionModel = groupModel.fieldList[j];
                    NSDictionary *dic = @{@"fieldId":sectionModel.indentifierID,@"content":sectionModel.value?:@""};
                    [aryEnd addObject:dic];
                }
            }
...
- (void)__getAllDatas
{
    ...
    [PPHTTPRequest CustomGetDetailWithParameters:@{@"id":self.detailModel.indentifierID} success:^(id response) {
        ...
        strongSelf.tfTelePhone.itextField.text = response[@"datas"][@"phoneNum"]?:@"";
        strongSelf.tfUserName.itextField.text = response[@"datas"][@"name"]?:@"";
        [strongSelf.btnSelectType setTitle:response[@"datas"][@"customerTypeName"]?:@"类型选择" forState:UIControlStateNormal];
        strongSelf.loanType = [response[@"datas"][@"customerTypeName"]intValue];
        strongSelf.currentTypeModel.indentifierID = response[@"datas"][@"customerTypeId"];
        if (strongSelf.aryloanState.count>strongSelf.loanType) {
            [strongSelf.btnSelectLoanState setTitle:strongSelf.aryloanState[strongSelf.loanType] forState:UIControlStateNormal];
        }
        for (int i = 0; i<strongSelf.aryGetTypeModel.count; i++) {
            ADCustomTypeModel *typeModel = strongSelf.aryGetTypeModel[i];
            if ([typeModel.indentifierID isEqualToString:response[@"datas"][@"customerTypeId"]]) {
                strongSelf.currentTypeModel = typeModel;
            }
        };
        [strongSelf __buildUI];
...
- (ADCustomTypeModel *)currentTypeModel
{
    if (_currentTypeModel == nil) {
        _currentTypeModel = [[ADCustomTypeModel alloc]init];
    }
    return _currentTypeModel;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.btnClick = ^(ADCellBtn * btn) {
        ...
        formCommonContoller.isEdit = YES;
