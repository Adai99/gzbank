...
#import "GaoDeiClusterAnnotation.h"
...
- (NSArray *)getAnnotationsWithoutClusteredInMapRect:(MAMapRect)rect
{
    ...
    QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(rect), ^(QuadTreeNodeData data) {
        ...
        GaoDeiClusterAnnotation *annotation = [[GaoDeiClusterAnnotation alloc] initWithCoordinate:coordinate count:1];
    ...
    for (NSInteger x = minX; x <= maxX; x++)
    {
        for (NSInteger y = minY; y <= maxY; y++)
        {
            ...
            if (count == 1)
            {
                ...
                GaoDeiClusterAnnotation *annotation = [[GaoDeiClusterAnnotation alloc] initWithCoordinate:coordinate count:count];
...
- (NSArray<GaoDeiClusterAnnotation *> *)clusteredAnnotationsWithinMapRect:(MAMapRect)rect withDistance:(double)distance {
    ...
    NSMutableArray<GaoDeiClusterAnnotation *> *clusteredAnnotations = [[NSMutableArray alloc] init];
    for (AMapPOI *aAnnotation in allAnnotations) {
        ...
        GaoDeiClusterAnnotation *cluster = [self getClusterForAnnotation:aAnnotation inClusteredAnnotations:clusteredAnnotations withDistance:distance];
        if (cluster == nil) {
            GaoDeiClusterAnnotation *aResult = [[GaoDeiClusterAnnotation alloc] initWithCoordinate:resultCoor count:1];
...
- (GaoDeiClusterAnnotation *)getClusterForAnnotation:(AMapPOI *)annotation inClusteredAnnotations:(NSArray<GaoDeiClusterAnnotation *> *)clusteredAnnotations withDistance:(double)distance {
    ...
    for (GaoDeiClusterAnnotation *aCluster in clusteredAnnotations) {
